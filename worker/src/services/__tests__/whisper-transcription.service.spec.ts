...
import fs from "fs";
import { IAudioChunkerService } from "../../interfaces/audio-chunker.interface";
...
describe("WhisperTranscriptionService", () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(fs.createReadStream).mockReturnValue(Readable.from(["test"]) as unknown as fs.ReadStream);
  ...
  it("should throw an error on catastrophic API failure (500 or Timeout) to trigger BullMQ retry", async () => {
    ...
    await expect(service.transcribe({ audioPath: "/tmp/audio.mp3" })).rejects.toThrow(
  ...
  it("should chunk audio and clean up chunk files if audio file is larger than 24MB", async () => {
    ...
    const result = await service.transcribe({ audioPath: "/tmp/large_audio.mp3" });
