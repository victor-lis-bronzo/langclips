...
export class FFmpegAudioExtractorService implements IAudioExtractorService {
	...
	private parseDuration(ffmpegOutput: string): number {
		const match = ffmpegOutput.match(
	...
	async extract({
		...
		if (totalDuration > MAX_DURATION) {
			...
			console.log(
				`[AUDIO-EXTRACTOR] Vídeo curto. Extraindo áudio completo de ${duration}s`,
			);
		}
		...
		return new Promise((resolve) => {
			...
			command
				...
				.on("end", () => {
					console.log(
						`[AUDIO-EXTRACTOR] Áudio extraído com sucesso com startOffset=${startOffset}s`,
					);
